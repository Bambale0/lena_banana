...
async def menu(message: Message, context: AppContext, state: FSMContext) -> None:
    await state.clear()
    user = await ensure_user_for_message(message, context)
    await message.answer("Главное меню", reply_markup=_main_menu_markup(user, context))
...
async def menu_callback(callback: CallbackQuery, context: AppContext, state: FSMContext) -> None:
    ...
    if callback.message:
        ...
        if callback.message.text in EMPTY_HOME_TEXTS:
...
async def _balance_text(user, context: AppContext) -> str:
    ...
    text = (
        ...
        f"Универсальные кредиты: <b>{int(user.credits_balance or 0)}</b>{unlimited}\n"
        f"Партнерский баланс: {affiliate:.0f} ₽\n"
        f"\n{generation_status}\n"
        "Партнерская программа: 30% с покупок приглашенных пользователей. "
        "Амбасадоры получают 50%. Хотите стать амбасадором? Напишите в поддержку."
    )
    if link:
        text += f"\n\nВаша ссылка:\n{link}"
    return text
