...
async def image_menu(message: Message, context: AppContext, state: FSMContext) -> None:
    await state.clear()
    await ensure_user_for_message(message, context)
    await _send_image_request_screen(message, state)
...
async def image_menu_callback(callback: CallbackQuery, context: AppContext, state: FSMContext) -> None:
    ...
    if callback.message:
        await _send_image_request_screen(callback.message, state)
...
async def _send_image_request_screen(message: Message, state: FSMContext) -> None:
    await state.set_state(ImageFlow.reference_prompt)
    await message.answer(
...
async def select_model(callback: CallbackQuery, context: AppContext, state: FSMContext) -> None:
    ...
    if model.category == "image":
        await state.update_data(model_code=model.code, user_id=user.id)
        if callback.message:
            await _send_image_request_screen(callback.message, state)
    else:
        await state.set_state(MotionFlow.image)
...
async def receive_image_reference(
    ...
    if not file_id:
        await message.answer("Нужна картинка JPEG, PNG или WebP.", reply_markup=navigation_keyboard())
        return
    data = await state.get_data()
    model_code = data.get("model_code")
    existing_prompt = str(data.get("prompt") or "").strip()
    incoming_prompt = (message.caption or "").strip()
    existing_references = _image_reference_items(data)
    if not model_code:
        async with session_scope(context.session_factory) as session:
            models = await list_enabled_models(session, "image")
        if not models:
            await message.answer("Сейчас нет включенных моделей изображений.", reply_markup=navigation_keyboard())
            return
        model_code = models[0].code
    status_message = await message.answer(_status_text("Принимаю референс", 15))
    async with session_scope(context.session_factory) as session:
        model = await get_model(session, str(model_code))
        fresh_user = await session.get(User, user.id)
        if not model or not model.is_enabled:
            await message.answer("Модель изображений сейчас недоступна.", reply_markup=navigation_keyboard())
            return
        limits = _generation_limits_payload(fresh_user or user, model)
        max_images = _max_image_references_from_limits(limits)
        if len(existing_references) >= max_images:
            await status_message.edit_text(
                f"Для {escape(model.title)} можно загрузить максимум {_references_count_text(max_images)}.",
                reply_markup=navigation_keyboard(back_callback="menu:image"),
            )
            return
        session.add(
    ...
    reference_items = _trim_image_reference_items(
        [
            *existing_references,
    ...
    await status_message.edit_text(
        _image_settings_text(await state.get_data()),
        reply_markup=_image_settings_keyboard(await state.get_data()),
    )
...
async def receive_image_settings_file(
    ...
    if not model_code:
        await message.answer(
            "Сначала выберите модель изображения.",
            reply_markup=navigation_keyboard(back_callback="menu:image"),
        )
        return
    ...
    async with session_scope(context.session_factory) as session:
        ...
        if not model or not model.is_enabled:
            await message.answer("Модель изображений сейчас недоступна.", reply_markup=navigation_keyboard())
            return
        max_images = _max_image_references_from_config(model.config)
        if len(reference_items) >= max_images:
            await message.answer(
                f"Для {escape(model.title)} максимум {_references_count_text(max_images)}.",
                reply_markup=navigation_keyboard(back_callback="menu:image"),
            )
            return
        session.add(
    ...
    incoming_prompt = (message.caption or "").strip()
    if incoming_prompt:
        updates["prompt"] = incoming_prompt
    await state.update_data(**updates)
    await _edit_image_settings_message(message.bot, message.chat.id, state)


@router.callback_query(ImageFlow.settings, F.data == "image:submit")
async def submit_image_settings(
    callback: CallbackQuery,
    context: AppContext,
    state: FSMContext,
    bot: Bot,
    ...
    if not str(data.get("prompt") or "").strip():
        await callback.answer("Сначала напишите промпт текстом", show_alert=True)
        return
    if not data.get("image_file_id"):
        await callback.answer("Сначала отправьте фото-референс", show_alert=True)
        return
    if not data.get("model_code"):
        async with session_scope(context.session_factory) as session:
            models = await list_enabled_models(session, "image")
        if not models:
            await callback.answer("Сейчас нет включенных моделей изображений", show_alert=True)
            return
        await state.update_data(model_code=models[0].code)
    await _submit_image_task(callback, context, state, bot)
...
def _image_settings_text(data: dict[str, Any]) -> str:
    ...
    if prompt:
        instruction = (
            "Выберите параметры кнопками ниже. Затем нажмите «Запустить» "
            "или отправьте новый промпт текстом, чтобы заменить текущий."
        )
    else:
        instruction = (
            "Выберите параметры кнопками ниже. Затем отправьте промпт обычным сообщением, "
            "и я сразу запущу генерацию."
        )
    if references_count < max_references:
        instruction = (
            f"Можно отправить еще референс до лимита {references_count}/{max_references}. "
            f"{instruction}"
        )
