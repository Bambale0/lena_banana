...
async def admin_cancel(callback: CallbackQuery, context: AppContext, state: FSMContext) -> None:
    ...
    await state.clear()
    if callback.message:
        await callback.message.answer("Действие отменено.", reply_markup=_admin_keyboard())
    await _safe_answer(callback)
...
@router.callback_query(F.data == "admin:users:grant")
...
@router.callback_query(F.data == "admin:users:block")
...
async def admin_broadcast_confirm(
    ...
    if not text:
        await _safe_answer(callback, "Текст рассылки потерян", show_alert=True)
        return
    async with session_scope(context.session_factory) as session:
        broadcast = Broadcast(
