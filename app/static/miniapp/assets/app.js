import {
  ...
  TONE_FRAGMENTS,
  getModel,
} from "./catalog.js?v=20260630-refcounts1";
...
const imageModels = [
  { code: "nano-banana", title: "banana · 2K", displayPrice: 10, quality: "2K", maxImages: 1 },
  { code: "nano-banana-2", title: "banana-2 · 2K", displayPrice: 15, quality: "2K", maxImages: 14 },
  { code: "nano-banana-pro", title: "banana-pro · 2K", displayPrice: 20, quality: "2K", maxImages: 8 },
];
...
function renderFeed() {
  return renderAppShell(`
    <header class="page-head">
      <div class="page-title">
        <h1>Лента</h1>
        <p class="subtle">Публичные работы BANANA</p>
      </div>
      <button class="ghost-pill" type="button" data-action="refresh-feed">Обновить</button>
    </header>
...
function renderFeedItem(item) {
  ...
  return `
    <article class="feed-card">
      ...
      <div class="feed-body">
        <div>
          <h3>${escapeHtml(item.author || "BANANA user")}</h3>
          <p>${escapeHtml(item.prompt || "без prompt")}</p>
          <small>${escapeHtml(item.model_code || "model")}</small>
        </div>
        <div class="feed-actions">
          <button type="button" data-feed-action="like" data-feed-id="${Number(item.id)}">♡ ${Number(item.likes) || 0}</button>
          <button type="button" data-feed-action="share" data-feed-id="${Number(item.id)}">↗ ${Number(item.shares) || 0}</button>
          <button type="button" data-feed-repeat="${Number(item.id)}">${item.media_type === "video" ? "Повторить видео" : "Повторить фото"}</button>
        </div>
      </div>
    </article>
  `;
...
function renderEmptyFeed() {
  return `
    <div class="empty-state">
      <h3>Пока пусто</h3>
      <p class="subtle">Опубликуйте готовую генерацию из Telegram-бота.</p>
    </div>
  `;
}
...
function generatorTitle() {
  ...
  return state.kind === "motion" ? "Создайте видео" : "Создавайте картинку";
}
...
function generatorSubtitle() {
  ...
  return "Воплощайте свои идеи в реальность";
}
...
function renderMediaHandoff() {
  ...
  return `
    <div class="handoff-card">
      ...
      <small>После брифа бот попросит ${escapeHtml(referenceLimitText(maxImages))} в Telegram-чате.</small>
    </div>
  `;
}
      ...
      <section class="sheet" role="dialog" aria-modal="true" aria-labelledby="sheetTitle">
        ...
        <div class="form-stack">
          ...
          <div class="sheet-actions">
            <button class="primary-button" type="button" data-action="send-generation">✧ Отправить бриф (${displayPriceText()})</button>
