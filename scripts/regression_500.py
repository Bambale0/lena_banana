...
def _check_static_logic(regression: Regression) -> None:
    ...
    for value in [None, "", "2K", "1K", "4K", "bad", 0, " 2K "]:
        ...
        regression.check(name, normalize_image_resolution(value) == "2K")
    ...
    for item in DEFAULT_MODELS:
        ...
        if item["category"] == "image":
            regression.check(name, item["config"].get("resolutions") == ["2K"])
            regression.check(
    ...
    for code in expected_codes:
        ...
        if model:
            if code == "seedance-2/video":
                ...
                regression.check(name, model.config.get("resolutions") == ["2K"])
                regression.check(
